...
@property (nonatomic, assign) CGPoint      additionalOffset;
...
- (void)TPKeyboardAvoiding_keyboardWillShow:(NSNotification*)notification {
    ...
    if ( firstResponder ) {
        CGFloat viewableHeight = self.bounds.size.height - state.additionalOffset.y - self.contentInset.top - self.contentInset.bottom;
        [self setContentOffset:CGPointMake(self.contentOffset.x + state.additionalOffset.x,
...
-(void)TPKeyboardAvoiding_scrollToActiveTextField {
    ...
    CGFloat visibleSpace = self.bounds.size.height - state.additionalOffset.y - self.contentInset.top - self.contentInset.bottom;
    
    CGPoint idealOffset
        = CGPointMake(self.contentOffset.x + state.additionalOffset.x,
...
- (void)TPKeyboardAvoiding_setAdditionalOffset:(CGPoint)additionalOffset {
    TPKeyboardAvoidingState *state = self.keyboardAvoidingState;
    state.additionalOffset = additionalOffset;
}
...
-(CGFloat)TPKeyboardAvoiding_idealOffsetForView:(UIView *)view withViewingAreaHeight:(CGFloat)viewAreaHeight {
    ...
    offset = subviewRect.origin.y - padding - (self.contentInset.top - state.additionalOffset.y);
    ...
    CGFloat maxOffset = contentSize.height - viewAreaHeight - (self.contentInset.top - state.additionalOffset.y);
    if (offset > maxOffset) {
        offset = maxOffset;
    }
